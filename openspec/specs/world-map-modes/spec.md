...
#### Scenario: Temperature mode uses perceived temperature
- **WHEN** `tileVisual` is called with `MapModeTemperature`
- **THEN** the returned char is `█` and the color reflects biome-adjusted temperature (deep ocean appears cooler than desert at same latitude)
