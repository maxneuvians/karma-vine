...
The system SHALL define: `Ground{Char rune, Color string, Passable bool}`, `Object{Char rune, Color string, Blocking bool, Name string}`, and `Animal{X, Y int, Char rune, Color string, Flee bool, Name string}`. `LocalMap` SHALL have fields `Ground [42][18]Ground`, `Objects [42][18]*Object`, `Animals []*Animal`.
